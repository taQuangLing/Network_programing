...
//                "Bong da"));
    ...
    public void followingOnClick(MouseEvent e) throws IOException {
        ...
        Data data = new Data(AppUtils.MessageCode.FOLLOWING);
        data.getData().add(1);
        AppUtils.sendData(clientSock, data);
        Data response = AppUtils.recvData(clientSock); // receive count
        int count = Integer.valueOf((String) response.getData().get(0));
        System.out.println(response);
        if(response.getMessageCode() == OK ) {
            userList = getUserList(count);
    ...
    public void followerOnClick(MouseEvent e) throws IOException {
        ...
        Data data = new Data(AppUtils.MessageCode.FOLLOWER);
        data.getData().add(1);
        AppUtils.sendData(clientSock, data); // send data
        Data response = AppUtils.recvData(clientSock); // receive count
        int count = Integer.valueOf((String) response.getData().get(0));
    ...
    public void initialize(URL url, ResourceBundle resourceBundle) {
        ...
        Data request = new Data(AppUtils.MessageCode.FRIENDS);
